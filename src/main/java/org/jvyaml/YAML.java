...
public class YAML {
    ...
    public static void dump(final Object data, final Writer output, final YAMLFactory fact,
    ...
    public static void dumpAll(final List data, final Writer output, final YAMLFactory fact,
            final YAMLConfig cfg) {
        final Serializer s = fact.createSerializer(fact.createEmitter(output, cfg), fact
                .createResolver(), cfg);
        try {
            s.open();
    ...
    public static Object load(final String io, final YAMLFactory fact, final YAMLConfig cfg) {
        final Constructor ctor = fact.createConstructor(fact.createComposer(fact.createParser(fact
                .createScanner(io), cfg), fact.createResolver()));
        if (ctor.checkData()) {
            return ctor.getData();
        } else {
            return null;
